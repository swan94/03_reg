...
-   Compare all models using AIC *(the lower this value, the better the model)*. Which one fits the data best? Choose one to proceed and use in the next steps.
...
Because both the response variable (i.e., corn yield) and explanatory variable (i.e., seeding rate) are **numerical**, we can analyze this in a **regression** approach (instead of ANOVA).  *USE OF ANOVA WHEN X VALUES ARE CATEGORICAL, since both x and y values are numerical we will be using regression analysis.*
